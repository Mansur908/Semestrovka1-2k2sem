...
public class RestLoginServiceImpl implements RestLoginService {
    ...
    @Override
    public boolean checkUser(RestLoginDto loginDto) {
        Optional<User> userOptional = userRepository.findByEmail(loginDto.getEmail());
        return userOptional.isPresent() && passwordEncoder.matches(loginDto.getPassword(),userOptional.get().getHashPassword());
    }
}
